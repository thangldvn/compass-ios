...
extension DegreeScaleView {
    ...
    private func configScaleDial1() {
        
        /// 360度
        let degree_360: CGFloat = 2 * CGFloat.pi
        
        /// 180度
        let degree_180: CGFloat = degree_360 / 2
        
        /// 角度
        let angle: CGFloat = degree_360 / 360
        ...
        var lastAngle: CGFloat = (3 * .pi / 2)
        
        for i in stride(from: 0, to: 360, by: 2) {
            let startAngle: CGFloat = lastAngle + angle * CGFloat(i)
...
//
//                let point: CGPoint = calculateTextPositon(withArcCenter: po, andAngle: textAngle, andScale: 1)
//                DrawTemple(point)
//            }
//            // 刻度的标注
            if (i % 30 == 0) {
                ...
                let textAngle: CGFloat = startAngle + (endAngle - startAngle)
...
//        for i in stride(from: 0, to: 360, by: 2) {
//            let startAngle: CGFloat = -(.pi / 2) + angle * CGFloat(i)
//
//            let endAngle: CGFloat = startAngle + angle
//            let bezPath = UIBezierPath(arcCenter: po, radius: frame.size.width / 2 - 70, startAngle: startAngle, endAngle: endAngle, clockwise: true)
//            let shapeLayer = CAShapeLayer()
...
//        画圆环，每隔2°画一个弧线，总共180条
        for i in 0 ..< 180 {
            ...
            // 创建一个路径对象
            let bezPath = UIBezierPath(arcCenter: po, radius: frame.size.width / 2 - 70, startAngle: startAngle, endAngle: endAngle, clockwise: true)
            print("THANGLD \(startAngle/Double.pi) \(endAngle/Double.pi)")
...
//            }
    ...
    private func DrawTemple(_ point: CGPoint) {
//        yelloTempleView = UIView(frame: CGRect(x: point.x, y: point.y, width: 12, height: 12))
//        yelloTempleView.center = CGPoint(x: point.x, y: point.y + 17)
//        yelloTempleView.backgroundColor = .clear
//        backgroundView.addSubview(yelloTempleView)
//
//        // 画三角
//        let trianglePath = UIBezierPath()
//        var point = CGPoint(x: 0, y: 0)
//        trianglePath.move(to: point)
//        point = CGPoint(x: 0, y: 12)
//        trianglePath.addLine(to: point)
//        point = CGPoint(x: 12, y: 12)
//        trianglePath.addLine(to: point)
//        point = CGPoint(x: 12, y: 0)
//        trianglePath.addLine(to: point)
//
//        trianglePath.close()
//        let triangleLayer = CAShapeLayer()
...
//        yelloTempleView.layer.addSublayer(triangleLayer)
        
        let iv = UIImageView()

        // Set image with image literal
        iv.image = UIImage(named: "box")
        iv.frame = CGRect(x: point.x, y: point.y, width: 12, height: 12)
        print("center of iv \(iv.center)")
//        iv.center = CGPoint(x: point.x, y: point.y + 17)
...
//        })
    ...
    func setBoxView(i: Int) {
        ...
        let lastAngle: CGFloat = (3 * .pi / 2)
        
        let startAngle: CGFloat = lastAngle + angle * CGFloat(i + 2)
            
        let endAngle: CGFloat = startAngle + angle
        
        let textAngle: CGFloat = startAngle + (endAngle - startAngle) / 2

        let point: CGPoint = calculateTextPositon(withArcCenter: po, andAngle: textAngle, andScale: 1.05)
        
        boxImageView.frame = CGRect(x: point.x, y: point.y, width: 12, height: 12)
        print("center of iv \(boxImageView.center)")
//        iv.center = CGPoint(x: point.x, y: point.y + 17)
        backgroundView.addSubview(boxImageView)
    }
    
    func processWhenHitQiblaDirection(isHitted: Bool) {
        DispatchQueue.main.async {
            self.lineView.backgroundColor = isHitted ? .green : .white
        }
    }
