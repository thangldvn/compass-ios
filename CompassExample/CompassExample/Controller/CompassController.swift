...
extension CompassController {
    ...
    func calculateX() {
           ...
           CLLocationManager.authorizationStatus() ==  .authorizedAlways
        {
            ...
            let nyc = Coordinates(latitude: currentLocation?.coordinate.latitude ?? 0.0, longitude: currentLocation?.coordinate.longitude ?? 0.0)
            qiblaDirection = Qibla(coordinates: nyc).direction
            print("currentQibla: \(Int(qiblaDirection))")
            dScaView.setBoxView(i: Int(qiblaDirection))
        }
    }
}
...
extension CompassController: CLLocationManagerDelegate {
    ...
    func locationManager(_ manager: CLLocationManager, didUpdateHeading newHeading: CLHeading) {
        ...
        if newHeading.headingAccuracy > 0 {
            ...
            // 设置角度label文字
            geographyInfoView.angleLabel.text = "\(Int(Double(magneticHeading) - qiblaDirection))"
            
            print("sacasc \(magneticHeading)")
            // 3.旋转变换
            ...
            if (Int(Double(magneticHeading) - qiblaDirection) == 0) {
                print("THANGLD: NENENENENNEEN")
                dScaView.processWhenHitQiblaDirection(isHitted: true)
            } else {
                dScaView.processWhenHitQiblaDirection(isHitted: false)
            }
        }
    }
